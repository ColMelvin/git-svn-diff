...
$repo->svn_new_file("file", "text contents");
$repo->svn_commit("Initial commit");
test_repo($repo, [qw(-c 1)], "Added a text file");

$repo->svn_remove_file("file");
$repo->svn_new_file("file", $black);
$repo->svn_commit("Replace with binary");
$repo->git_run(qw(git checkout -q git-svn));

$TODO = "Issue #19" if cmp_ver(get_svn_version(), '1.7') >= 0;
$TODO = "Issue #13";
test_repo($repo, [qw(-c 2)], "Replaced file with binary");
